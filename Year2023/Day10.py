lines = """-J.|..|7F-J7.J7F7.-77LJ-FL-F77F77F7|F-FLF7.F.FL7F-J.|.FF7-|-F7F7F7.FF-F7LL7FF-J7.----77-FF-..JF7.L-.FF|-F7--|7F77L7FF77F77.FL-7-L.FL7.FF-7..
...
def part1():
    ...
    max_i = len(lines_split)
    max_j = len(lines_split[0])
    ...
    def move_from_pos():
        for p in list(pos_to_dist):
            ...
            if p[0] - 1 >= 0:
                # North
                if lines_split[p[0] - 1][p[1]] in ["|", "F", "7"]:
                    pos_to_dist[(p[0] - 1, p[1])] = d + 1
            if p[1] - 1 >= 0:
                # West
                if lines_split[p[0]][p[1] - 1] in ["-", "F", "L"]:
                    pos_to_dist[(p[0], p[1] - 1)] = d + 1
            if p[0] + 1 < max_i:
                # South
                if lines_split[p[0] + 1][p[1]] in ["|", "J", "L"]:
                    pos_to_dist[(p[0] + 1, p[1])] = d + 1
            if p[1] + 1 < max_j:
                # East
                if lines_split[p[0]][p[1] + 1] in ["-", "J", "7"]:
                    pos_to_dist[(p[0], p[1] + 1)] = d + 1
                   
        return len(pos_to_dist) != 0
    
    
    keep_going = True
    while keep_going:
        keep_going = move_from_pos()
    
    
    print(max(dist_to_pos.values()))
